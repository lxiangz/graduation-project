...
var families=['手机电话','电脑宽带','房款房贷','物业','住宿房租','材料建材','家政服务','快递邮政','生活其他','美发美容','保险费','消费贷款','税费手续费'
  ,'漏记款','婚庆摄影','生活费'];
...
export default{
  data(){
    return{
      payMenu:[{text:'餐饮'}, {text:'交通'}, {text:'购物'}, {text:'娱乐'}, {text:'医教'}, {text:'居家'}, {text:'投资'}, {text:'人情'}, {text:'生意'}//一级菜单
      ],
      payDetails:foods//当前选择的一级菜单
    }
  },
  methods:{
    back(){
  ...
  mounted:function () {
    console.log("555");
    if(this.$store.state.page=="record"){//一级菜单显示如图，不包含全部一项
      this.payMenu=[{text:'餐饮'}, {text:'交通'}, {text:'购物'}, {text:'娱乐'},
        {text:'医教'}, {text:'居家'}, {text:'投资'}, {text:'人情'}, {text:'生意'}
      ];
      if(foods.length>4){//如果二级菜单数据更改过，重新设置更新数据，不包括记账页面已存在的支出项。
        foods.splice(0,6);traffics.splice(0,1);shops.splice(0,1);
        plays.splice(0,1);medical_educations.splice(0,1);families.splice(0,2);
        invests.splice(0,1);human_feelings.splice(0,1);business.splice(0,1);
      }
    }
    if(this.$store.state.page=="detail"){
      console.log(foods);
      this.payMenu=[{text:'餐饮'}, {text:'交通'}, {text:'购物'}, {text:'娱乐'}, {text:'医教'},
        {text:'居家'}, {text:'投资'}, {text:'人情'}, {text:'生意'}, {text:'全部'}//一级菜单加上全部项
      ];
      if(foods.length==4){//如果耳机菜单不全，则补全数据，并加上全部项
        foods.unshift('餐饮全部','早餐','午餐','晚餐','饮料水果','买菜原料');traffics.unshift('交通全部');
        shops.unshift('购物全部');plays.unshift('娱乐全部');
        medical_educations.unshift('医教全部');families.unshift('居家全部','水电燃气');
        invests.unshift('投资全部');human_feelings.unshift('人情全部');
