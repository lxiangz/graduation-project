/**
 * Created by 程序小姐 on 2017/6/12.
 */
class Clip{
  //因为要保存到vue实例中，所以直接传一个$vm当闭包使用了
  constructor(wpId,$vm){
    this.regional = document.getElementById(wpId);//容器
    this.getImage = document.createElement('canvas');//绘制canvas
    this.getImage.id = 'image-box';
    this.editBox = document.createElement('canvas');//编辑canvas
  ...
  init(url){
    this.sx = 0; //裁剪框的初始x
    this.sy = 0; //裁剪框的初始y
    this.sWidth = 200; //裁剪框的宽
    this.sHeight = 200; //裁剪框的高
    this.chooseBoxScale = 200/200;

    this.handleFiles(url);
  }
  handleFiles(url){
   let t = this;//该对象
    t.imgUrl=url;
  ...
  save(){
    ...
    images.onload = function(){
      ...
      t.drawImageIOSFix(ctx, images,cropWidthScale * t.sx , cropHeightScale* t.sy,
        t.sWidth * cropWidthScale, t.sHeight * cropHeightScale, 0, 0, 400, 400);
